...
- In RTL tests, never write `expect(screen.getBy*(...)).toBeDefined()` — all `getBy*` queries (`getByRole`, `getByText`, `getByLabelText`, etc.) already throw if the element is absent, so `.toBeDefined()` is unconditionally true and asserts nothing about the element's content. Assert `.textContent`, `.value`, or a specific attribute instead (e.g. `expect(screen.getByRole('status').textContent).toBe('Changes saved successfully.')`)
