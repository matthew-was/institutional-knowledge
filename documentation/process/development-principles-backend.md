...
This file covers backend-specific implementation patterns for `apps/backend/`.
...
Dependencies are composed at the top of the call chain (`server.ts`) and then narrowed as they
are passed down. Each factory receives only the interface it actually uses.
...
The same rule applies to services: a service that only needs `db`, `storage`, and `config`
should declare exactly those three parameters, not an opaque "deps" object that happens to
contain them. Keeping the surface area minimal makes mock injection in tests trivial and makes
dependencies self-documenting.

---

## Service Pattern

Services are **factory functions**, not classes. Methods are closures over the dependency
arguments.
...
- Methods return `ServiceResult<T, K, E>` for all expected outcomes — never throw for domain
  errors such as "not found" or "file too large". Throwing is reserved for genuinely unexpected
  failures (DB unreachable, programming error) that should propagate to the Express error
  handler via `next(err)`. `T` is the success data type; `K extends string` is the union of
  valid `errorType` strings (defaults to `string`); `E` is the type of `errorData` for
  structured error payloads (defaults to `never` for methods whose error cases carry only a
  message).
- Services have no knowledge of Express (`Request`, `Response`, `NextFunction` are route-layer
  concerns).
- The exported interface type is what callers depend on; the factory implementation is an
  internal detail.
- **`ServiceResult` inside transactions**: when calling a function that returns
  `ServiceResult<T, E>` from within a `db._knex.transaction()` block, always check `outcome`
  immediately. If `outcome === 'error'`, throw an error to trigger rollback — do not allow the
  transaction to commit despite a logical failure. Discarding a `ServiceResult` return value
  inside a transaction is prohibited.
...
All SQL lives in `apps/backend/src/db/repositories/`. Repositories are grouped by **domain**,
not by table — a repository owns all the tables that belong to its domain and may JOIN across
them freely for read queries, and may run transactions across them for writes. Services call
`db.graph.addTermWithRelationships(...)` or `db.documents.insert(...)` — they never write SQL
directly and never call `db._knex` except to open a transaction boundary that crosses repository
domains (which should be rare and explicitly justified).
...
`knex.raw()` is permitted inside repositories for queries that cannot be expressed in the Knex
query builder (pgvector cosine distance, recursive CTEs). When using `knex.raw()`, column names
must be written in snake\_case because `wrapIdentifier` does not apply; result rows must be
mapped to camelCase explicitly.

**Transaction participation**: repository methods that may be called inside a caller-owned
transaction accept an optional `trx?: Knex.Transaction` parameter and use `const qb = trx ?? db`
internally. Any abstraction that wraps a repository (e.g. `VectorStore`, `GraphStore`) must
expose the same optional `trx` parameter on methods that write to the database, and thread it
through to the underlying repository call. Omitting `trx` from an abstraction's write method
forces the write onto a separate pool connection, which cannot see uncommitted rows from the
outer transaction and will fail FK constraints.

---

## Backend Testing Strategy — Two Tiers
...
this is not a contract type that API consumers need to know about.
...
**Frontend schema rules**: the frontend has additional rules about how it consumes shared
schemas and which form-validation schemas are permitted to live in `apps/frontend/src/lib/schemas.ts`.
See `development-principles-frontend.md` (Schema placement — frontend rules section).
...
Never log document content, file content, or user-provided text at any level. Log only
identifiers (uploadId, documentId, chunkId) and status values. This is a security boundary —
see Production-Ready Patterns in `development-principles.md`.

---

## ADR Citation Standard

Every source file in `apps/backend/src/` should include a one-sentence ADR citation in its
file-level JSDoc. The format varies by file type:

- **Interface files** (`StorageService.ts`, `VectorStore.ts`, `GraphStore.ts`): cite the ADR
  that defines the abstraction.
...
- **Concrete implementations** (`LocalStorageService.ts`, `PgVectorStore.ts`,
  `PostgresGraphStore.ts`): cite as "implements ADR-XXX".
...
- **Repository files**: cite only if there is a non-obvious design decision documented in an
  ADR (e.g. the graph repository cites ADR-037 for the document-evidenced filter). Omit the
  citation if there is no relevant ADR.
...
primary key performance is not a concern.

---

## What These Principles Rule Out (Backend)
...
| Calling a service factory with mocked `db`/`storage` deps as a "unit test" | The mock bypasses real I/O, leaving the HTTP boundary and validate middleware untested | Test Early |
| Inline `res.status(...).json({ error, message })` in a route handler instead of `sendServiceError` | The envelope shape is a project rule; inlining it creates drift risk | Error Response Pattern |
| Inline conditional to compute HTTP status from `errorType` instead of a `Record<ErrorType, number>` map | The `Record` form is TypeScript-exhaustiveness-checked | Error Response Pattern |
| Omitting `trx` from an abstraction's write method when the underlying repository supports it | Forces the write onto a separate pool connection; FK violations on uncommitted rows | Repository Pattern / Transaction Participation |
| Discarding a `ServiceResult` return value inside a `db._knex.transaction()` block | Transaction commits despite a logical failure; errors are silently swallowed | ServiceResult Pattern |
| A service function writing to two or more tables without a single `db._knex.transaction()` wrapping all writes | Partial writes leave the database in an inconsistent state | Repository Pattern / Transaction Participation |
| Hardcoding operational numeric limits in Zod schemas in `packages/shared/src/schemas/` | Embeds a backend-specific constraint in the shared API contract | Infrastructure as Configuration |
| Using `?? ''` to substitute empty string for null in repository row-mapping code | Converts meaningful absence into indistinguishable empty string | Type Safety |
| Constructing services individually in integration tests instead of using `createTestApp()` | Boilerplate that must be updated in every test file when `AppDependencies` gains a new entry | Test Early |
| A service operation that interleaves storage I/O inside a `db._knex.transaction()` block | Storage cannot participate in a SQL transaction and will not roll back; use the three-step sentinel pattern instead | Repository Pattern / Transaction Participation |
| Synchronous `fs.*` calls | Blocks the Node.js event loop; use `node:fs/promises` | Production-Ready Patterns |
| Instantiating a concrete service implementation directly in integration tests instead of using the factory function | Bypasses `storage/index.ts`; factory goes untested | Infrastructure as Configuration |
| `crypto.randomUUID()` for UUID generation | The `uuid` package (`v7 as uuidv7`) is the project standard | Consistency |
| Adding a repository to `DbInstance` without updating the `DbInstance` listing in `development-principles-backend.md` | The listing is the authoritative reference for what is on `DbInstance` | Documentation |
