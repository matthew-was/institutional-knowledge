...
graph LR
    ...
    subgraph Processing["Python Service"]
        Processor["Processing<br />Pipeline"]
    end
...
graph TD
    ...
    subgraph Python["Python Processing Service - ADR-015, ADR-038"]
...
Embedding, vector search, RAG, citations. **Phase 1 focus**: vector retrieval via QueryRouter (pass-through) and VectorStore. Graph-aware retrieval via GraphStore (ADR-037) is deferred to Phase 2, where QueryRouter becomes an LLM classifier (ADR-040) and can route to graph or hybrid vector-graph paths.
...
graph TD
    ...
    subgraph QueryFlow["Query Processing - C3"]
        QueryEmbed["Query Embedding<br />Same model as C2<br />ADR-024"]
        VectorStore["VectorStore Interface<br />ADR-033"]
    ...
    subgraph Routing["Query Routing - ADR-040"]
        QueryRouter["QueryRouter Interface<br />Phase 1: vector only<br />Phase 2: LLM classifier"]
    end
    ...
    VectorSearch --> Rerank
    DB -->|"Document metadata<br />+ chunk text"| Rerank
    FileStore -.->|"Source documents<br />(Phase 2)"| Rerank
    Rerank --> RAG
    RAG --> Response
    Response --> API
    API -->|"Response"| CLI
